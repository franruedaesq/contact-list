...
const ContactCardItem = ({ title, content }) => {
  return (
...
const ContactCardTitle = ({title, showTitle = true}) => (
  <h3
...
export default function ContactCard({ data, activeId, index }) {
  ...
  return (
    ...
    >
      <ContactCardTitle title={`${name} ${lastName}`} showTitle={showCard} />
      <div
      ...
      >
        ...
        <div>
          ...
          {phone && <ContactCardItem title="Phone:" content={phone} />}
          {streetNumber && (
            <ContactCardItem
              title="Street:"
              content={`${streetNumber} ${street}`}
            />
          )}
          {city && <ContactCardItem title="City:" content={city} />}
          {state && <ContactCardItem title="State:" content={state} />}
          {postCode && <ContactCardItem title="Postcode:" content={postCode} />}
        </div>
      </div>
      <button
